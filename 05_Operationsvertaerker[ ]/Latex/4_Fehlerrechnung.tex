...
Der Mittelwert $\mean{x}$ aus mehreren Ergebnissen $x_{i}$ einer Messung 
wurde mit Hilfe von 
...
\end{empheq}
berechnet.
Für die Berechnung der statistischen Abweichung $\sigma_{x}$ wurde die folgende Gleichung verwendet:
\begin{empheq}{equation}
...
angenommen.
Für die Fehlerfortpflanzung dieser Unsicherheiten $\sigma_{x_i}$  wurde die 
gaußsche Fehlerfortpflanzung verwendet.
Damit berechnet sich der Fehler $\sigma_y$ einer Größe $y = y(\va{x})$, mit den Messgrößen $\dim{\va{x}} =: n$,
wie folgt:
\begin{empheq}{equation}
...
Die relative Abweichung $\Delta_{\mathrm{rel}}x$ eines Messergebnisses $x$ vom gegebenen 
Theoriewert~$x_{\mathrm{theo}}$ wurde mit folgender Gleichung berechnet:
\begin{empheq}{equation}
